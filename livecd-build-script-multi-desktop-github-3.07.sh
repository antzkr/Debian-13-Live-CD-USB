...
chroot_path() {
    local de="${DE:-}"
    [ -z "$de" ] && return 1
    echo "$BUILDF/$LIVE_BOOT/$de/chroot"
}

# Unmount virtual filesystems inside the chroot (robust)
unmount_vfs () {
    local root
...
cleanup () {
    ...
    set +e
    unmount_vfs
    unset PASS1 USER1 CHRUSER1 2>/dev/null
    exit "$rc"
}
...
build_iso () {
    ...
    mkdir -p "$BUILDF/$LIVE_BOOT/$DE/staging/EFI/BOOT"
    mkdir -p "$BUILDF/$LIVE_BOOT/$DE/staging/boot/grub/x86_64-efi"
    mkdir -p "$BUILDF/$LIVE_BOOT/$DE/staging/boot/grub/themes/debian13"
    mkdir -p "$BUILDF/$LIVE_BOOT/$DE/staging/isolinux"
    mkdir -p "$BUILDF/$LIVE_BOOT/$DE/staging/live"
    mkdir -p "$BUILDF/$LIVE_BOOT/$DE/tmp"
...
alias gd-open='rclone mount gdrive:/ ~/gdrive/remote --vfs-cache-mode full --vfs-cache-max-age 24h --vfs-read-ahead 256M --vfs-read-ahead 256M --transfers 4 --daemon'
#alias gd-mount='gocryptfs ~/gdrive/remote ~/gdrive/mount && echo -e "\nGoogle Drive remote folder mounted\n"'
alias gd-close='fusermount3 -u ~/gdrive/remote && echo -e "\nGoogle Drive remote folder unmounted\n"'
alias m-open='rclone mount mega:/ ~/mega/mount --vfs-cache-mode full --vfs-cache-max-age 24h --vfs-read-ahead 256M --vfs-read-ahead 256M --transfers 4 --daemon'
alias m-close='fusermount3 -u ~/mega/mount && echo -e "\nMega remote folder unmounted\n"'
alias f-open='rclone mount filen:/ ~/filen/mount --vfs-cache-mode full --vfs-cache-max-age 24h --vfs-read-ahead 256M --vfs-read-ahead 256M --transfers 4 --daemon'
alias f-close='fusermount3 -u ~/filen/mount && echo -e "\nFilen remote folder unmounted\n"'

alias lsblk='lsblk -e 179'
alias cont-open='cont'
alias cont-close='cont_close'
SKEL_EOF


    # Add personal folders to user home
    mkdir -p /etc/skel/{scripts,gdrive/{remote,mount},mega/{remote,mount},filen/{remote,mount}}
    ...
    unzip DejaVuSansMono.zip  -x "*.txt" "*.md"
    unzip JetBrainsMono.zip  -x "*.txt" "*.md"
    unzip RobotoMono.zip  -x "*.txt" "*.md"
    rm -f *.zip 

    # Install icons-in-terminal (full terminal icons support)
    cd /tmp
    git clone https://github.com/sebastiencs/icons-in-terminal.git
    cd icons-in-terminal
    bash install.sh

    mkdir -p /home/${USER1}/.config/fontconfig/conf.d
    cat > /home/${USER1}/.config/fontconfig/conf.d/30-icons.conf << 'IIT_EOF'
<?xml version="1.0"?>
...
<fontconfig>
  ...
  <alias><family>JetBrainsMono Nerd Font Mono,JetBrainsMono NFM,JetBrainsMono NFM Light</family><prefer><family>icons-in-terminal</family></prefer></alias>
</fontconfig>
IIT_EOF
...
# Fixed syntax recommendations
# Fixed path variable errors
# Fixed mkdir errors
# Simplified adding password to user in chroot (no hashing)
# Added robust chroot VFS cleanup
# Enable errexit/pipefail so ERR trap fires reliably
