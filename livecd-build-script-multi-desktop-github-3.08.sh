...
build_iso () {
    ...
    mkdir -p "$BUILDF/$LIVE_BOOT/$DE/staging/boot/grub/fonts"
...
# Plain black background (grub theme handles the rest)
set gfxmode=auto
set gfxpayload=keep
...
GRUB_EOF
    ...
    if ! command -v convert &> /dev/null; then
        ...
        apt install imagemagick -y -q 2>/dev/null || {
            echo -e "${RED}Failed to install ImageMagick. Skipping ISOLINUX background.${NC}"
        }
    fi
    ...
    if [ -f /usr/share/grub/unicode.pf2 ]; then
        cp /usr/share/grub/unicode.pf2 "$BUILDF/$LIVE_BOOT/$DE/staging/boot/grub/fonts/"
    else
