...
echo -e "- Includes RAM optimization\n"
echo -e "See README.md for details.\n"
echo -e "${YELLOW}Note:${NC} Loading the entire filesystem to RAM increases"
echo -e "performance but at the cost of using more RAM space (not"
echo -e "recommended for systems with less than 8GB RAM).\n"
echo -e "${CYAN}=========================================================${NC}\n"
...
env_select () {
    echo -e "\n${YELLOW}Choose target desktop environment for Live CD/USB build:${NC}\n"
    PS3="Option: "
    select DE in "CLI" "KDE" "GNOME" "MATE" "XFCE" "Exit"; do
        if [[ "$DE" == "Exit" ]]; then
...
preseed_user_pass () {
    ...
    while [[ $attempts -lt $max_attempts ]]; do
        echo -e "Enter password for ${CYAN}'$USER1'${NC}: "
        read -sp "" PASS1
        ...
        if [[ "$PASS1" == "$PASS2" ]]; then
            ...
            if [[ $attempts -lt $max_attempts ]]; then
                echo -e "${RED}✗ Password does not match.${NC} You have $((max_attempts - attempts)) attempts left.\n"
            else
...
chroot_access () {
    ...
    if [ $CHROOT_EXIT -eq 0 ]; then
        ...
        PS3="Option: "
        select CHOICE2 in "Rebuild Live CD/USB iso" "Exit"; do
            if [[ "$CHOICE2" == "Exit" ]]; then
                unmount_vfs
                echo -e "${BLUE}Script will exit.${NC}\n"
                exit 1
            elif [[ "$CHOICE2" == "Rebuild Live CD/USB iso" ]]; then
...
build_iso () {
    ...
    # Compress filesystem with squashfs
...
menuentry "Debian 13 Live - Normal Boot [Keep USB]" --class debian {
    ...
    initrd ($root)/live/initrd
    echo "Loading changes into RAM (filesystem read-only). USB must remain connected..."
}
...
GRUB_EOF
    ...
    # Generate boot menu backgrounds only if convert is available
    if command -v convert &> /dev/null; then
        ...
        # Alternative background: solid color compatible with most systems
    ...
    ######################################
    # Copy Bootloader Files to workspace #
    ######################################
    ...
    if [ -f /boot/memtest86+.bin ]; then
        ...
        touch "$BUILDF/LIVE_BOOT/$DE/staging/live/memtest86+.bin" # Failsafe memtest86+ creation
    fi
...
#######################################
# Execute build update selection loop #
#######################################
...
hashed_pass() {
    ...
    #########################
    # Customizing user home #
    #########################

    # Create skeleton dir to load for all users
    mkdir -p /etc/skel

    # Customize .bashrc here
...
echo && echo && toilet -f smblock -w 80 -F metal "\$USER Live"
echo
echo "=========================================="
...
alias sn='sudo nnn "-eocHi"' # run nnn file manager as root
SKEL_EOF

    # Add personal folders to home dir here
    #mkdir -p /etc/skel/{your-custom-folders}

    # Set correct permissions in skeleton dir
...
######################################
# Execute building Live CD/USB iso #
######################################
unmount_vfs
build_iso
exit 1
